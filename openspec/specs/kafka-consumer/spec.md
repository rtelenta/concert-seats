...
Provides a NestJS injectable `KafkaConsumer` service that subscribes to Kafka topics, validates incoming messages, deduplicates by `eventId`, extracts W3C trace context from message headers, and commits offsets only after successful handler execution.
